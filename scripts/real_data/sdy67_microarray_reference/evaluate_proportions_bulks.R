...
output_dir <- file.path(
    ...
    "real_data", paste0("sdy67_microarray_reference_", train_size, "train_", test_size, "test_", num_runs, "run")
)
...
for (irun in 1:num_runs) {
    ...
    proportion_file <- file.path(output_dir, paste0("proportions_all_lm22_run_", irun, ".rds"))
...
if (!file.exists(file.path(output_dir, "proportions_cibersort_sdy67_lm22.rds"))) {
    stop("Error: The CIBERSORTx results file 'proportions_cibersort_sdy67_lm22.rds' not found.")
}
proportions_cibersort <- readRDS(file.path(output_dir, "proportions_cibersort_sdy67_lm22.rds"))
...
irun <- 1 # this run is the closest to the mean in this case
exclude <- c("true", "harp")
...
for (algo in algos) {
    ...
    print(one_sided_z_test(proportions_all = proportions_all, algos = c("cibersort_harp_lm22", algo), irun = irun))
...
if (!file.exists(file.path(output_dir, "bulk_correlation_cibersort_sdy67_lm22.rds"))) {
    stop("Error: The CIBERSORTx results file 'bulk_correlation_cibersort.rds' not found.")
}
correlations_bulk_cibersort <- readRDS(file.path(output_dir, "bulk_correlation_cibersort_sdy67_lm22.rds"))
...
harp_ref <- c("cibersort_rna", "cibersort_harp_lm22", "bp_subtypes", "bp_harp", "cibersort_lm22")
...
ggsave(file.path(stats_dir, "zimmerman_lm22_vertical_include.pdf"),
    combined_plot,
    device = cairo_pdf,
    width = 600,
    height = 1200,
...
combined_plot_reduced <- (plot_list_main$individual_plots$R + plot_list_main$sample_wise) /
    (plot_list_main$celltype_wise + bulk_correlations_plot_main) /
    (box_plot + plot_spacer()) / # you can add the umap plot here
    plot_layout(heights = c(1, 1, 1, 1, 1, 1), guides = "collect") +
    # Specify equal widths for all columns
    plot_annotation(tag_levels = "a") &
    theme(
...
ggsave(file.path(stats_dir, "zimmerman_lm22_vertical_reduced.pdf"),
