...
train_size <- 20 # update this for the number of train samples #20
...
for (irun in 1:num_runs) {
    ...
    cibersort_harp_lm22 <- paste0("CIBERSORTx_Results_harp_run_", irun, ".txt") # add the name of file of cibersortx output when using the reference from harp (when using lm22 as anchor) for example "cibersortx_job3.txt"
...
saveRDS(cibersort_prp, file.path(output_dir, "proportions_cibersort_sdy67_lm22.rds"))
saveRDS(cibersort_bulk_correlations, file.path(output_dir, "bulk_correlation_cibersort_sdy67_lm22.rds"))
