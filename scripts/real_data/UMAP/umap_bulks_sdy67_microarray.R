...
output_dir <- "data/generated/real_data/sdy67_microarray_reference_150train_100test_5run"
# make sure you have the correct dir here for different runs with different train and test samples
...
irun <- 1 # this is the closest run to the average — it is the most representative run.
...
predicted_bulk_expression_harp_train <- harp_ref %*% zimmermann_data$bulk_pheno[celltype, train.samples]
...
predicted_bulks_sc <- reference.profile %*% zimmermann_data$bulk_pheno[celltype, ]
...
# save the plot
ggsave(file.path(stats_dir, "umap_zimmerman_lm22_harp_predicted.pdf"),
